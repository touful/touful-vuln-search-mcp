...
from src.config import KEV_CATALOG_URL, KEV_CACHE_FILE, KEV_CACHE_TTL_HOURS
from src.clients.base import BaseAPIClient
from src.clients.http_utils import resilient_request
from src.cache import load_json_cache, save_json_cache, is_cache_fresh
...
class KEVClient(BaseAPIClient):
    ...
    async def load_catalog(self) -> list[dict]:
        """获取 CISA KEV 全量数据（缓存优先，不阻塞）。

        先尝试从本地磁盘缓存加载，命中则直接返回；
        未命中或缓存已过期则返回空列表（不发起网络请求），
        由服务层在后台异步刷新。
        ...
        Returns:
            ...
            缓存不可用时返回空列表 []。
        """
        data = await self.load_from_cache()
        if data is not None:
            return data
        return []

    async def load_from_cache(self) -> list[dict] | None:
        """从本地磁盘缓存加载 KEV 数据。

        Returns:
            vulnerabilities 列表，缓存不存在或已过期返回 None。
        """
        data = await load_json_cache(KEV_CACHE_FILE)
        if data is None or not isinstance(data, list):
            return None
        if not is_cache_fresh(KEV_CACHE_FILE, KEV_CACHE_TTL_HOURS):
            logger.info("KEV 缓存已过期，需要后台刷新")
            return None
        logger.info("KEV 从磁盘缓存加载，共 %d 条", len(data))
        return data
